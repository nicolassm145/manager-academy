...
export const getTransactions = async (filters: TransactionFilters = {}): Promise<Transaction[]> => {
  ...
  const url = `${API_BASE_URL}/transacoes/listar${params.toString() ? `?${params.toString()}` : ''}`;
  ...
  return transactions.map((t) => ({
    ...t,
    equipe: t.equipeId ? (teams.find((team) => String(team.id) === String(t.equipeId))?.nome || "") : "",
  }));
};
...
export const createTransaction = async (
  ...
  const body = {
    ...transaction,
    tipo: transaction.tipo.charAt(0).toUpperCase() + transaction.tipo.slice(1).toLowerCase(),
  };
...
export const updateTransaction = async (
  ...
  const body = {
    ...data,
    ...(data.tipo ? { tipo: data.tipo.charAt(0).toUpperCase() + data.tipo.slice(1).toLowerCase() } : {}),
  };
